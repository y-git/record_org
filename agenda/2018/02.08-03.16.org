* 今日计划
** DONE multi thread test
CLOSED: [2018-02-12 Mon 20:25]





** TODO finish shaders.glsl change
...
////*** TODO user added worker job should use thread safe api(xxxWorker api?)
no need safe api!!!

user job can change data directly, while other worker jobs should read copied sharedArrayBuffer data!
...
** DONE pass multi thread render render draft test
CLOSED: [2018-02-14 Wed 10:02]



** DONE microservice architecture refactor
CLOSED: [2018-03-15 Thu 09:04]

setting service

*** TODO user should use api
user should use it instead of job service even when add custom job




*** TODO Editor refactor
move data(editor/engine state) to logic_service->composite service->data

not distinguish engine,editor,database service

facade is one to one(to logic_service-> composite service, service)



move ui state to be service:
ui global state
ui local state
...
*** DONE extract job service
CLOSED: [2018-03-15 Thu 08:01]
finish cameraController:
rename CameraControllerJobSingleService

extract PerspectiveCameraFrustumJobSingleService
(
has its own index; add create function; 

move pMatrixMap, dirtyArray to it;

move update logic to it:
CameraControllerJobSingleService remove update logic;
change Init/UpdateCameraControllerJob to Init/UpdateCameraJob
)



upgrade XXXCommon to XXXJobSingleService


extract common service:
move addToDirtyArray to it



////extract CameraControllerJobCompositeService
...
finish gameObject:
extract composite service(ignore other components except cameraController, PerspectiveCameraFrustum)



refactor:
////rename gameObjectData to gameObjectContainerData

////job service:
GameObjectContainerXXXService

////aggr service:
GameObjectXXXService
...
test:
////add scene service test

add perspective camera test(redo, ...)
add scene test(redo, ...)


pass all integration test
...
//refactor:
////state: remove "record/data" post fix!!!(e.g. rename gameObjectRecord to gameObject)
不加数据类型的后缀！
...
//refactor:
move test->transformTool to be TransformServiceTool
move test->geometry tool to be Geometry service tool
...
////refactor:
rename ViewXXX to ScreenXXX
move contextConfig from view data to deviceManager data
...
//render worker:
state service add post fix: "Main", "Render"





//rename StateDataType to MainStateDataType

//base service add service:
getIsDebugFromStateData(StateData.stateData.isDebug)




refactor:
//remove GlType->options
(duplicate with ContextType)



//move glsl to be src/glsl/
//move worker to be src/worker/
...
refactor:
//rename renderConfigData to renderConfigRecord
...
refactor:
//move worker->xxx to worker->jobs/
...
////export render service to api facade?
...
*** DONE extract primitive service
CLOSED: [2018-03-15 Thu 08:01]

move to it:
structure->ArraySystem
ComponentSystem


*** DONE extract atom service
CLOSED: [2018-03-15 Thu 08:49]
create Wonder-Engine-Service project:
move used WonderCommonlib->files to it





*** DONE refactor folder
CLOSED: [2018-03-15 Thu 08:59]
src/:
service/
(move state to state_service->data)
(move StateSystem to state_service)
(move StateSystem->restore to service->stateTuple->RestoreEngineService?)

job/(all jobs here)
api/
////config/
(json data related here. e.g. parse logic)
(move src/config/ files here)
(

rename data/ to config/
...
*** DONE refactor
CLOSED: [2018-03-15 Thu 09:03]
//add getExn service



//rename GameObject->get component to unsafeGetXXX


//unsafeGetGameObjectSourceInstanceComponent should use Js.Option.getExn



*** DONE pass u,i,e2e test
CLOSED: [2018-03-15 Thu 09:04]
...
** DONE dispose gameObject keep order(need engine fix)
CLOSED: [2018-03-15 Thu 11:00]
description
dispose gameObject cause order wrong(scene tree)

reason


solution
add disposeGameObjectKeepOrder api in engine
...
////bsconfig.json remove "commonjs" ?:
fix test pf,render(gulp file -> require)
...
** DONE solve code climate
CLOSED: [2018-03-15 Thu 19:49]



** DONE update Wonder-Editor
CLOSED: [2018-03-16 Fri 11:25]
...
//pass one engine test

//pass one editor test
...
**** DONE fix dispose keep order bug
CLOSED: [2018-03-16 Fri 11:25]

**** DONE check "dispose bug" is still exist?
CLOSED: [2018-03-16 Fri 11:19]
description
add two boxes(one camera?)
dispose two boxes one by one
add one boxes
error: no binded buffer!
...
*** TODO worker
**** TODO support user add worker job(in main/render/camera worker)
copy transform data
...
*** TODO advanced: gpu cull(need compute shader)
