...
** TODO add worker job data
*** DONE finish draft
CLOSED: [2018-02-19 Mon 18:40]

add isUseWorker to workerDetectData

finish useWorker


*** DONE demo
CLOSED: [2018-02-25 Sun 16:09]

**** DONE worker->init
CLOSED: [2018-02-24 Sat 21:40]
//draft
...
**** DONE worker->draw
CLOSED: [2018-02-25 Sun 16:09]
create wonder-most-animation-frame project and import
...
*** DONE rearrange plan
CLOSED: [2018-02-26 Mon 10:00]
use trello:
write down engine,editor tasks together

////when finish one, edit it!




*** DONE no worker->init, loop
CLOSED: [2018-02-28 Wed 07:40]
//pass compile



**** DONE init
CLOSED: [2018-02-27 Tue 14:02]

***** DONE pass u/i test
CLOSED: [2018-02-27 Tue 14:02]
//pass load test

//add set setting test

//add init device test

//init jobs:
add initLightMaterialJob_test
add pregetGLSLDataJob_test
...
*** TODO finish worker->init
**** TODO pass worker test demo
...
test create worker instance




**** TODO finish init render:
pass compile:
create canvas
...
**** TODO debug info use debug instead of log


*** TODO pass unit,integration test

**** TODO add worker unit test
...
ecs admin and renderer admin is for job
...
(api->GameObject.re use it instead of use GameObjectAdmin)


Job should use ecs admin instead of ecs system
(e.g. InitCameraControllerJob.re)



Job should use renderer admin instead of renderer system
(e.g. use GLSLSenderAdmin instead of GLSLSenderDrawUtils, ...)
...
*** TODO finish worker->draw
**** TODO make transform data safe
copy transform data to draw buffer
...
*** TODO finish no worker->init, draw
...
*** TODO refactor
clean worker,single data


add getExn in OptionUtils.re instead of Js.Option.getExn:
use fatal!
(correspond change Wonder-Editor!)



most.js:
add "callFunc" api (refer to animation-frame)



unify specification:
all json data use "xxx_xxx"
all json->record type use "xxxXXX"


rename "JobConfig" to "JobConfigData"



remove DebugUtils.re
...
////move all main config to setting.json:
...
** TODO publish
restore test coverage to 95% up:
////restore ci test coverage limit(.travis)
restore ci test coverage limit(jest_coverage->not ignore worker files)
...
*** TODO finish worker->init
*** DONE pass worker test demo
CLOSED: [2018-03-18 Sun 08:27]

//pass init test demo


//pass init render test
...
*** DONE create gl in render worker
CLOSED: [2018-03-19 Mon 17:24]
...
*** DONE change to arraybuffer
CLOSED: [2018-04-02 Mon 13:34]

//transform:
add buffer config to setting
...
//test:
////add sharedBoxGeometry_test

////add sharedBasicMaterial_test

//refactor XXXMaterial_test -> dispose->group count
...
*** DONE add custom geometry
CLOSED: [2018-03-31 Sat 11:35]
//add 
...
//box geometry not reallocate!

////reset type array when dispose box geometry!

remove set points api!




//pass box geometry test
...
//custom geometry need reallocate!

not reset type array when dispose custom geometry!


remain set points api!



//fix "create custom geometry from disposedIndex":
should from it after reallocate?

add test cases with custom geometry, box geometry(integration test)




add custom geometry u,i test:
//CustomGeometry_test
//gameObject
//clone
//redo undo
...
add test case for custom geometry


//fix bug

//reallocate currentGeometryMap
...
unit test:
//pass dispose box, custom geometry->test vbo buffer

integration test:
//current ->/* TODO test vbo buffer */

//add send custom, send basic:
bind vbo test cases
create different buffer



//test last send geometry
...
//optimize gameObject:
remove boxGeometryMap, customGeometryMap

fix add,batchAdd, dispose, batchDispose component
(
e.g. remove _addSharableComponent but remain increase group count?
)


//use get geometry instead of get box/custom geometry component
//type_ should be "box"/"custom"


////rename currentGXXX to geometry




refactor:
//change geometry component data to (index, type_, mapTuple, funcTuple)
...
cloneXXX_test:
//clone material->add share material cases


//clone geometry-> add share geometry description
...
refactor:
//rename init_geometry job to init_boxGeometry
...
*** DONE no worker-> render object buffer
CLOSED: [2018-04-03 Tue 10:56]
...
////change instance to array buffer
...
refactor:
//remove renderRecord->renderArray



//u,i test:
fix loop->pipelines,jobs
...
fix material:
////last Send material should judge type_
(add test cases)
...
optimize gameObject:
when add material component, decide gameObject is in basic or light render array!

remove basicMaterialMap, lightMaterialMap, use currentMaterialMap instead?

use get material instead of get basic/light material component
...
*** DONE pass basic demo
CLOSED: [2018-04-09 Mon 20:22]
**** DONE init
CLOSED: [2018-04-08 Sun 18:00]


init basic material:

//get render config data
...
use "convert state to render data" :
...
pass no worker->u,i test:
//pass others

add test "update transform job":
//test dispose case
...
//use getSourceInstanceObjectInstanceArray
...
////glsl sender not use state, use type arrays directly

remove get data for glsl sender->uniform




fix render:
render, sendAttribute, sendUniform states




finish:
//no worker->init material->build init material all state data
...
refactor:
remove not-used get data in component service(e.g. OperateBasicMaterialMainService.getColor?)




/*
update all transforms in main worker job



no worker and worker:
////update all transforms([0, index - 1]) before render
update all rendered transforms before render
...
//finish worker-> init basic material
...
*** DONE refactor
CLOSED: [2018-04-09 Mon 20:30]

//remove ColorMapService



//move gameObjectRecord from MainStateDataType to GameObjectType


*** TODO optimize no worker
**** DONE render
CLOSED: [2018-04-11 Wed 10:19]
...
pass worker run test:
//renderRecord->basic buffer
...
////if render objects not change, remain last render command buffer?
else if change, only operate the changed part in last render command buffer?
...
remove array buffer
only has one geometry data
only has one geometry buffer


test vertices, ...


//remove box geometry buffer count
...
***** TODO optimize render worker
//optimize create basic render object buffer

////fix normalMatrixCacheMap?


//pass localToWorldMatrixCacheMap?
...
***** TODO pf test
////light box should fail!!!



**** DONE static instance
CLOSED: [2018-04-11 Wed 10:32]


**** DONE render basic+light+box/custom geometry+not clone
...
***** DONE pf test add cases?
CLOSED: [2018-04-11 Wed 11:28]



**** DONE transform
CLOSED: [2018-04-11 Wed 16:28]
transform+set parent

**** DONE dispose:
CLOSED: [2018-04-11 Wed 18:42]
//test memory
////memory leak?
...
//pf test->add cases:
dispose not cloned ones
add dispose basic+ customGeometry case
...
**** DONE dynamic instance
CLOSED: [2018-04-11 Wed 18:56]
...
**** DONE redo_undo
CLOSED: [2018-04-12 Thu 17:19]
//deepCopy(then solve editor perf!!!):
...
optimize copy, restore


copy: not copy typearrays
...
pass redo undo case:
create 10 arraybuffers when idle
...
////test box + custom geometry:

fix custom geometry 
(refer to box geometry)
(all state)



*** DONE fix pf test
CLOSED: [2018-04-12 Thu 19:24]
fix :
if testDataList: [
{
test1...
...
all fail, should report test1 and test2 fail!!  not only test1 !!!



*** DONE pf test
CLOSED: [2018-04-12 Thu 21:52]
...
reason:
CreateBasicRenderObjectBufferTypeArrayRenderWorkerJob-> geometryTypes, sourceInstances error



*** DONE refactor worker
CLOSED: [2018-04-13 Fri 11:39]
//remove duplicate



*** DONE finish worker feature
CLOSED: [2018-04-28 Sat 10:15]

**** TODO transform
***** DONE add job
CLOSED: [2018-04-16 Mon 11:02]
...
//finish:
exclude worker_pipellines->get,set worker data jobs and create_gl job

test by init and loop pipelines job like no worker!!!




//fix render basic->model matrix
...
//rename test->initSystemAndRender to init
...
***** DONE optimize render basic
CLOSED: [2018-04-16 Mon 15:40]


////why "addEvent" take much time???
...
////***** TODO add "copy_basic_material" job
...
***** DONE add worker job
CLOSED: [2018-04-17 Tue 10:37]



////test use transfer array buffer instead of sab
...
//main loop:
use RenderJobsRenderWorkerTool
...
****** DONE refactor
CLOSED: [2018-04-17 Tue 10:37]
////RenderJobRenderWorkerTool


//remove duplicate

//rename No/WorkerJobService to XxxJobMainService


***** DONE fix operate no worker job
CLOSED: [2018-04-17 Tue 10:40]


////***** TODO add safe API
add TransformSafeAPI


////add requireCheck for unsafe api?
...
***** DONE add sync job
CLOSED: [2018-04-17 Tue 11:15]

////add integration test:
can add worker job into sync phase which is safe!
...
**** DONE create and dispose
CLOSED: [2018-04-23 Mon 07:20]
...
//batch for keep order


//remove dispose, only remain batch dispose
...
worker:
//test send message
//test dispose vbo in render worker




////test sourceInstance->dispose geometry->vbo buffer


//test geometry->vbo buffer:
box 
...
refactor:
//remove DisposeXXXComponentTool
...
//1.create and dispose:
all be mutable operation(record mutable)

//2.basic material array for init: should remove disposed ones!



3.with no dispose, only create-> render created one which isn't inited yet!


//clear init material array after send?
...
/*
set shader index:
set shader index in initGameObject

not send shaders,shader libs to render worker
*/



//remove getShaderIndex in XxxMaterialMainService.re
...
////4.render->shader index in render array buffer should exist!
...
u,i test:
//test "if shaderIndex not exist(is default value)(means that render worker not finish init shader), not draw"
...
refactor:
////main workekr job remove return value?!
...
//fix wonder-log->assertNotEqual->message: use not equal message





////worker setting add "dispose_defer_frame_count"(default is 2)



***** DONE optimize create and dispose
CLOSED: [2018-04-23 Mon 07:20]
...
*** DONE pass light demo
CLOSED: [2018-04-24 Tue 21:50]


**** TODO init
***** DONE add logic
CLOSED: [2018-04-23 Mon 20:23]

////***** TODO test
add test case

**** DONE render
CLOSED: [2018-04-23 Mon 21:37]
...
////**** TODO refactor
split to RenderBasic, FrontRenderLight state from Render state
...
////add init light material case

//disposeRenderWorker_test, initMaterialRenderWorker_test  add cases


//add render test case



////add copyTransformMainWorkerJob_test test case
...
*** DONE optimize light
CLOSED: [2018-04-25 Wed 10:06]
**** TODO create and dispose
...
*** DONE finish customGeometry
CLOSED: [2018-04-25 Wed 15:50]
//change infoArray to typeArray


////get,set add getIndex logic


test:
////get custom data with two custom geometrys
...
*** TODO pass instance demo

**** TODO render basic

////**** TODO refactor
split to RenderBasic->instance, FrontRenderLight->instance state from Render state
...
***** DONE test
CLOSED: [2018-04-27 Fri 15:20]
//pass test


//init basic material->instance shader
...
//solve create and dispose(fix run bug)!!!!
...
//dispose these in render worker:
...
//test dispose sourceInstance in no worker



***** DONE fix get type array when create instance buffer
CLOSED: [2018-04-27 Fri 15:46]
...
***** DONE refactor
CLOSED: [2018-04-27 Fri 16:38]
//render worker add "get_material_data" job;
create init basic/light material state only with render worker state!
...
**** DONE front render light
CLOSED: [2018-04-27 Fri 17:23]
...
*** DONE refactor
CLOSED: [2018-04-27 Fri 17:24]
rename setWorkerData to setDataWorker



///*** TODO optimize instance
...
check all->create and dispose->memory leak:
no worker
...
send objectInstanceTransformArray:
filter only when need
...
use debug instead of print(e.g. print send matrix3 data) to print info for render worker debug!
...
test localToWorldMatrixCacheMap
...
*** TODO add contract check: different lines' job shouldn't set the same record of the main state

*** TODO add e2e test for multi thread ?
...
init:
glsl should contain light count
...
** DONE refactor
CLOSED: [2018-04-28 Sat 11:00]
//rename test->xxxToolWorker to xxxWorkerTool
...
////debug info use debug instead of log




** DONE finish all todos
CLOSED: [2018-04-29 Sun 18:06]


** DONE optimize
CLOSED: [2018-04-30 Mon 14:10]
give benchmark report


*** DONE instance
CLOSED: [2018-04-29 Sun 17:22]
...
add test case:
////ObjectInstance_test ->"remove from sourceInstance->objectInstanceTransformCollections": add more cases

send render data->objectInstanceTransformIndexMap
...
**** DONE improve
CLOSED: [2018-04-29 Sun 17:20]
main, render worker job now not mutable state, just set state:
main worker jobs' merged job(get message job) shouldn't operate state

render worker jobs should all be concat:
main should wait finish_dispose message from render worker



**** DONE refactor
CLOSED: [2018-04-29 Sun 17:22]
rename TypeArrayService->get/setIntxxx to get/setUIntxxx
...
*** DONE create and dispose
CLOSED: [2018-04-30 Mon 10:11]

////why exceed 700?

why meshrenderer->batchRemoveFromArray 180w!!!???
...
** DONE refactor
CLOSED: [2018-04-30 Mon 14:24]
add no worker job: add before/after action




** DONE increase test coverage
CLOSED: [2018-04-30 Mon 15:07]
restore test coverage to 95% up:
restore ci test coverage limit(jest_coverage->not ignore worker files)
...
test StateAPI->get/setXXXFrom/ToData, createStateData


** DONE pass e2e test
CLOSED: [2018-04-30 Mon 15:49]
...
** DONE wonder-commonlib
CLOSED: [2018-04-30 Mon 15:57]

SparseMap add "t" for type



** TODO pass ci
deploy file->wd.renderWorker.js


** TODO publish


//copy e2e->json to data/


//fix gulp buildAll:
build rollupRenderWorker
...
** TODO update engine version

** TODO optimize redo-undo

copy array buffers when idle
...
** TODO diferent loop can use diferent job pipelines!

so when edit, loopBody should only update camera;
when run, loopBody should update all(include update collider, update script, ...).


////** TODO engine: add deepCopyForNew


** TODO add "contract check after set state" to ensure editEngineState and runEngineState should be the same

add isStateEqual(state1, state2) function!



** TODO test "refresh engine"


** TODO use GPL license


** TODO update wonder.js version

* 明日计划
** TODO fix all engine bug issue



** TODO update editor->engine version
...
** TODO fix for snake
*** DONE use old version
CLOSED: [2018-04-25 Wed 23:02]
//pass run test
...
fix add no worker job: use action type




add BasicCameraViewAPI->setBasicCameraViewWorldToCameraMatrix




fix  unsafeGetProgram bug
...
* TODO 未来7日计划
** TODO optimize
glsl source use Number() to flatten when shader source?
...
** TODO refactor
SparseMap, HashMap add t for type:
related fix wonder.js, wonder-editor


** TODO contract check
update cameraController->check "should has one camera at least"
