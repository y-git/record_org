* 今日计划
* 完成事项
////** TODO support .json(.wd) loader

////** TODO rewrite hongbao project(add more effect, show three cats)
** DONE use browserify + typescript(cmd)
CLOSED: [2017-02-06 Mon 07:09]
*** DONE support nodejs, cmd
CLOSED: [2017-02-04 Sat 11:20]
//fix cb,frp(add module.exports; add require; use browser)

//handle cannon(require it!)
...
package:(solve how to import file by user)
////animation add "skinSkeleton"(lite, custom)

////package lite,custom should clean dist/ folder firstly




//support cmd,nodejs build:(study lafite project)
debug.js,innerLib.js,all.js,lite.js,custom.js should all add corresponding version
script support cmd,nodejs version
fix Variable.ts: fix Promise when require!




//Frp.node.js require Cb.nodejs!
Frp add core.node.js version
...
update cb,frp->nodejs package(test tool->converter)
//add node.d.ts
(ignore lib/, test/, build/, src/)

//rename wdxxx to wonder-xxx


//wonder.js: add author,email(packcage, banner)


//wdfrp->node add node_modules->wdcb,wonder-package

//wdfrp->require("wdcb")


use npm instead(if no npm, remain submodule)(Wonder-Package only npm)
////(.gitignore: not contain Wonder-xxx submodule)
(browserify contain debug? innerLib)
(innerLib get from npm)
...
fix:
////wdFrp add .core.js
//wonderjs->combineInnerLib should use xxx.node.js(should judge require) file to combineInnerLib wd.xxx.innerLib.js


//refactor wonder:require

//rename browserify: "wd" to "wonder"

////can browserify singleon export wd,wdCb,wdFrp?



//move bowser,rsvp,chai,cannon to npm(remove from lib)
////using Typings to install d.ts file

//publish wdcb,wdfrp, rsvp,cannon d.ts


//tsconfig.json->filesGlob not need contain @types/xxx.d.ts?




*** DONE refactor package link point in src
CLOSED: [2017-01-22 Sun 18:19]
see references
...
////assemble logic to component?(need more abstract)

////e.g. move ModelGeometry->MorphBufferContainer logic to MorphAnimation?
(MorphAnimationData?)


////**** TODO add buildForCI(not browserify) and ci use it



////**** TODO packageCustom:move light out


////** TODO publish to README.org(move to Wonder-Application project)(need replace model!)

** DONE begin rewrite engine(v1.0.0-alpha.1)
CLOSED: [2017-02-26 Mon 18:00]
*** DONE extract min size of engine
CLOSED: [2017-02-06 Mon 07:19]
can draw multi boxes


*** DONE wonder-commonlib use es6 import
CLOSED: [2017-02-19 Sun 10:10]
**** DONE write script:
CLOSED: [2017-02-17 Fri 13:49]
remove module
...
//if has same name of different files, record to one file, and manual fix it
...
////umd->typescript sourcemap?


//todo: move to npm(can exec "wonder-tool-convertNamespaceToES2015 --xxx)


//format generated es2015 files

//can install -g when npm install this published package(preinstall?prepublish?)
...
////publish npm(bin?)
e.g.
wonder-tool-convertNamespaceToES2015

move to wonder-package as gulp task
...
//add gulp task for rollup,compile ts
//remove old build



//add format typescript gulp task when build
(move to wonder-package)




//update npm
(add module/jsnext and test it!)

//add d.ts for namespace

////test xxx.umd.js->commonjs(by browserify)


//publish to npm


*** DONE wonder-frp use es6 import
CLOSED: [2017-02-21 Tue 20:36]
script support:
////decorator
//nodejs package

//handle "import xxx = wdCb.xxx;"
...
//fix wonder-commonlib:
mv .ts to dist/es2015
use rollup-typescript


////refactor:
move "define rollup.config.js" to wonder-package



//remove submodule, use wonder-commonlib npm package instead

//publish npm

*** DONE wonderjs use es6 module
CLOSED: [2017-02-26 Sun 19:46]
use es6 import:
use Rollup


Similarly, the various cases of object instanceof THREE.Mesh and so on have been replaced with object && object.isMesh
(and the relevant constructors have been augmented such that this.isMesh is true for all instances of Mesh. This also
eliminates some nasty cyclical dependencies. 




pass tsc:
//add bowser->json data

//remove wdCb.$BREAK

//handle clone.ts->wd:
//add @registerClass by convert
//ClassUtils add "getClass" by class name
//ClassUtils add "classMap"
//fix clone->wd

//fix wonder-package->tsfmt:
limit file?
use glob?

//fix wonder-commonlib,wonder-frp
...
//fix wonder-frp->index.d.ts:not import module


//fix chai:
convert chai from commonjs to es2015?(use rollup plugin)

//npm install @types/node --save



//only import wonder-commonlib one time!
...
//solve typescript and duplicate problem!

//fix ClassUtils->getClassName


//add ENDLOOP,STARTLOOP,Transform event


//fix gulp task
execute prepareForRollup.js in "rollup" gulp task

support compress function name

////add compress task


//fix ShaderChunk
remove ShaderChunk->"body:xxx," the last comma!


//fix wonder-commonlib,wonder-frp->formatTs
...
//add es2015 sample test

//wonder-frp:not bundle dependencies wonder-commonlib!





////continue reduce src file!
should reduce wd.js lines < 15000




//write script to auto generate:
remove module
class: export, import
decorator
lib(cannon,expect,rsvp,bowser,wonder-xxx)
glsl
...
//remove rebundant examples->demos,samples,asset

wonder-xxx add nodejs dist files:

//add wonder-commonlib,wonder-frp -> xxx.commonjs.d.ts

//wonder-tool-converter,-common: add typeRoots:wonder-commonlib,wonder-frp



//Wonder.js,wonder-tool-converter:
rename @require,@requireInNodejs to @requireCheck

//wonder-tool-converter,-common:
change chai to expect.js


//npm publish wonder-tool-converter and pass -g test



//update wonder-xxx package
...
////refactor wonder-tool




manual fix:
need use local instead global(e.g. rewrite Variable.ts)


//strip glsl
See this demo vs this one (note the import * and individual export var declarations) for an example of how shaders could
be refactored to permit tree-shaking.  

//rebuild ShaderChunk.ts
each shader lib only import its glsl from ShaderChunk.ts
...
////how to refactor assembler?


refactor link point:
remove ClassUtils

user should register light parser to LoaderManager?
(so can avoid import light related code from WDParser/Assembler?)

////or user should register xxx module/xxx lib?
////(if not register, use the default import)
(register exclude_module/lib? just strip it fromm defaultly all registered)



extension:
move shadow back to src/



refactor isXXX(is class):
judge className
...
////add className(use script)(use compile api?)
regard it as abstract attribute in Object,Component

refactor clone->get class Name
refactor uglify: compress function name
...
integrate tsc,rollup to gulp


use one command to prepare all environment!
...
implement conditional import when typescript support "Proposal: Conditional Compilation":
https://github.com/Microsoft/TypeScript/issues/3538

e.g. user can run "gulp packageES6 --excludeModules action,animation --excludeLibs contract
in WDAssembler.ts:
#if !EXCLUDE_MODULE_ANIMATION:
import {MorphAnimation} from "./MorphAnimation"
import {SkinSkeletonAnimation} from "./SkinSkeletonAnimation"
#endif




use typescript compile api:
remove @require/xxx code if exclude contract lib
...
add wonderjs->npm,jspm package
package.json add "--typeRoots" field
pass d.ts file to Definitely repository
...
remove keyword
remove containDTS(manage exlude module lib)

fix packageLite,packageCustom

refactor Wonder-FRP->combineInnerLib


remove Wonder-commonlib/FRP->xxx.node.d.ts

wonder-package:
add d.ts->add reference typings!(wonder-frp, wonderjs)
/// <reference types="wonder-commonlib" />



fix browserify:now contain nodejs content



move converter, render test



install wonderjs package for wonder.js
add types field
(test Typings)



filter files instead of fileGlobs?



(bundleXXX)(chai move to bundle)

fix package:
remame module->chai to contractCheck
if not contain module->contractCheck, set isCompileTest to be false
(move to func logic)



runtime use require? how to package?
(refer to threejs, hilo)
(google: package)
...
add wd.xxx.node.js(only export wd)
wonder->node add node_modules->wonder-cb,wonder-frp,wonder-package
(ignore lib/, test/, build/, src/)
...
refactor packageLite:use packageCustom to rewrite


Cb,Frp build remove rebundant "__extend"

Cb.node.js remove debug.map



//extract compile common code to new submodule project from wdcb,wdfrp,wonder
...
*** TODO update wonder-xxx definite dts

update d.ts to definite repo:
add wonder-expect.js d.ts
add wonder-core d.ts
update wonder-xxx


////*** TODO contract check:change to "it"

*** DONE move tool to npm/submodule
CLOSED: [2017-02-26 Sun 19:31]
//fix wdfrp->global type


**** TODO pass tool->converter
**** TODO pass tool->terrain




*** DONE publish(v1.0.0-alpha.1)
CLOSED: [2017-02-26 Mon 18:00]

////explain nodejs version(import {wd,wdCb,wdFrp} = require("wd.all.js");)

**** DONE publish npm package(wonderjs?wonder-runtime?wonder-core?)
CLOSED: [2017-02-26 Sun 19:46]


//move tool->converter,terrain to npm!

**** DONE fix bower version
CLOSED: [2017-02-26 Sun 19:46]
ignore wd.xxx.node.xxx
...
**** TODO update document(es6 module!)
write why not import from "npm package"
(
if index.ts export the class which extend parent class,
__extend func cause tree shake not work! 
...
write how to import from "npm package"
(add "types" in tsconfig to refer to "xxx.noDelcareModule.d.ts)

write how to import from "sub package"
...
* Record
** rollup
*** if use rollup-plugin-typescript, then
rollup.config.js should in the dir of node_modules
or
the imported ts file should under current project dir

* TODO unsolved problem
** npm
*** TODO install wonder-tool-height_map_generator fail!
description
when exec:
npm install -g wonder-tool-height_map_generator

error:
npm ERR! enoent ENOENT: no such file or directory, chmod
'/usr/local/lib/node_modules/wonder-tool-height_map_generator/bin/generateHeightMap' 




reason
maybe the wonder-tool-height_map_generator->canvas npm cause the problem!
(canvas use node-gyp, and it cause the problem?)
...
this solution work:
npm install -g --no-bin-links wonder-tool-height_map_generator

but the bin isn't work then!
(command "generateHeightMap" not found!)
...
* TODO need improve
** rollup
*** TODO tree shake don't work well
description
in wonder-frp, we expect:
import { DomQuery } from "wonder-commonlib";

but it will include "Collection" code from "wonder-commonlib"!


reason
wonder-commonlib->index.ts export Collection, and the "__extend" function->
"(function(){xxx})()"
 cause the problem!


solution
import DomQuery from submodule:
import { Collection } from "wonder-commonlib/dist/es2015/Collection";


expection
import DomQuery from whole module:
import { DomQuery } from "wonder-commonlib";


so if the DomQuery's file path change, the user which import it can not do any change!



*** DONE the imported npm module will be included multi times if parent and submodule both import it
CLOSED: [2017-02-25 Sat 13:18]
refer to https://github.com/rollup/rollup-plugin-node-resolve/issues/80


description
Wonder.js import wonder-commonlib,wonder-frp, and wonder-frp import wonder-commonlib, so the wonder-commonlib's code
will be imported two times in the bundled file in Wonder.js by rollup!


current solution
so maybe we must ensure that the 'plugin-test1','plugin-test2' npm package shouldn't has the 'base-library' in "dependencies" field in package.json!
that means remove the 'base-library' from "node_modules" dir in 'plugin-test1' and 'plugin-test2' npm package!


expection
remain wonder-frp dependent on wonder-commonlib!



new solution
wonder-frp remove package.json->bundledDependencies->wonder-commonlib, so that the wonder-frp/node_modules will not
contain wonder-commonlib! 

so it's no need to remove wonder-commonlib manually!
