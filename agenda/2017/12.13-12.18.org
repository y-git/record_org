...
* 完成事项
** DONE support shared material
CLOSED: [2017-12-14 Thu 08:22]
*** DONE cloneGameObject add isSharedMaterial
CLOSED: [2017-12-13 Wed 12:18]
cloneGameObject = (gameObject: gameObject, count: int, isSharedMaterial:bool, state: StateDataType.state)


test:
dispose groupCountMap
...
test:
only send uniform color once



** DONE learn editor extend
CLOSED: [2017-12-14 Thu 22:00]


** DONE implement "restore from state"
CLOSED: [2017-12-18 Mon 21:16]
*** DONE implement "whole restore from state"
CLOSED: [2017-12-18 Mon 20:49]
//pass unit test
...
optimize:
//when restore: move current buffer,typeArr to pool?
(copy from pool)
...
//optimize:
instance: add pool(refactor with geometry)
...
instance
                  ...
                  //fix: shader restore: add index


//finish test:
not disable if no enable(0)

////fix:
when restore, why buffer 72 arraybuffer and 36 index buffer?

//pass: benchmark_redo_undo


////test: now clean all shader related data when restore
...
//fix different shader bug: 
test(use program):
send each shader's data
disable
...
//memory leak:
gameObject data->component,dispose
...
benchmark test(test shader)


optimize:
//use getShaderParameter only when contract check

//collect program, shader to pool?
(at least dispose program?)




//geometry: not deep copy(only shallow copy) verticesMap, indicesMap
////cameraController, transform: not deep copy(only shallow copy) type arr?
...
//refactor: rename restoreFromState to restore

//refactor: split Redo_undo_test.re



refactor: 
//rename deepCopyState to deepCopyStateForRestore

////change to array(Js.Undefined.t(xxx))
////remove Obj.magic when dispose related

//refactor: rename RedoUndo_test to redoUndo_test?
...
** DONE add sinon_test.re for wonder-editor
CLOSED: [2017-12-18 Mon 10:35]
...
*** TODO add event
**** TODO discrete system???not use EventManager? 
refer to cocos creator
http://docs.cocos.com/creator/manual/zh/scripting/events.html

这样的事件分发方式从集中由 cc.eventManager 分发，变为了拥有事件的节点本身分发自己的事件，可以说是集中式事件系统向离散式事件系统的转变。
