...
** TODO publish v0.5.8
* 完成事项
** DONE continue optimize(v0.5.8)
CLOSED: [2016-06-10 Fri 20:37]
*** DONE code refactor
...
**** DONE add refresh rate for framebuffer(shadow...)
CLOSED: [2016-06-02 Thu 12:13]
//refreshRate:
Use 0 to render just once, 1 to render on every frame, 2 to render every two frames and so on... 

//default value should be 1

////mirror,cubemap should be 1

//modify shadow


//refactor:
SceneDispatcher add "shadowMap" attr
rename SceneDispatcher->shadowMap, ShadowMapModel to shadowMapConfig/ShadowMapConfig


//pass unit test


**** DONE Using the hardware scaling feature (user can define in config)
CLOSED: [2016-06-02 Thu 17:09]
refer to https://blogs.msdn.microsoft.com/eternalcoding/2012/03/22/unleash-the-power-of-html-5-canvas-for-gaming/#hs

And once again, the canvas has a solution! The following code shows you how to use the hardware acceleration to rescale
the internal working buffer of a canvas to the external size of the DOM object: 
...
It is worth noting the difference between the size of the DOM objet (canvas.style.width and canvas.style.height) and the
size of the working buffer of the canvas (canvas.width and canvas.height). 

When there is a difference between these two sizes, hardware is used to scale the working buffer and in our case it is a
excellent thing: we can work on a smaller resolution and let the GPU rescales the result to fit the DOM object (with a
beautiful and free filter to blur the result). 
...
previous one. Even when the VAOs are different, setting them with
...
//fix change custom buffer data bug
...
//fix fire procedural bug

//add dispose


//procedural texture not need and not support vao?

////together with index buffer?
...
//rename Program->sendAttributeData to sendAttributeBuffer



//test performance in benchmark
pass 1000_light test
...
////**** TODO shader lib add cache
no longer to iterate the libs if not dirty

just send result vao or vbos/ uniform data
...
**** DONE optimize from <<WebGL Insights>>
CLOSED: [2016-06-05 Sun 07:46]
...
**** DONE update typescript version
CLOSED: [2016-06-05 Sun 14:18]


**** DONE 改进GameObject->update:
CLOSED: [2016-06-09 Thu 18:22]
...
//separate uiobject->update and render
//add update
...
//UIObjectScene not bind its own startloop,endloop


//pass unit test


//batch action for uiobject and gameobject


//batch script

////EntityObject add uidStr cache
//remove EntityObject->onStartLoop,onEndLoop
...
//remove beforeUpdateChildren,afterInitChildren

//remove after/before engine event





optimize benchmark_1000_basic:
optimize action
