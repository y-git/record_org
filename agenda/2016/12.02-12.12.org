...
* 完成事项
** DONE reorganizing data
CLOSED: [2016-12-02 五 10:32]

** DONE ticket, screen
CLOSED: [2016-12-02 五 09:38]

** DONE learn blogs
CLOSED: [2016-12-11 一 12:00]

** DONE prepare hilo3d
CLOSED: [2016-12-12 一 17:44]
*** DONE move min files from wonder.js to hilo3d
CLOSED: [2016-12-04 日 14:07]
**** DONE push first commit
CLOSED: [2016-12-04 日 14:07]
//change namespace to Hilo
...
////separate to core/ and other/ folder
...
*** DONE prepare specification(document(english), code, test)
CLOSED: [2016-12-04 日 14:41]


*** DONE prepare join document
CLOSED: [2016-12-04 日 15:17]
environment prepare

develop flow(runtime, tool):
typescript prepare
code quality requirements
git flow(fetch->rebase ...)
...

*** TODO prepare develop plan(runtime, tool)(check with jiang)


*** TODO prepare learn references(for inner runtime,tool develop)
add hilo3d ata
write files here



*** DONE cut canvas renderer
CLOSED: [2016-12-06 二 08:52]
learn canvas renderer implement
...
*** DONE communicate
CLOSED: [2016-12-06 二 08:52]
communicate with jiang,mo,yuan

@all
target(mobile compatible, high performance, hudong priority, advanced(pc first demo)), quality require
total work(canvas renderer, tool, docs, examples cases, package, mobile), everyone role/task, check point
work flow(merge request, commit, specification, test)
...
*** DONE reduce .min < 500kb
CLOSED: [2016-12-12 一 16:18]
**** DONE support package all and package lite
CLOSED: [2016-12-08 四 11:32]
...
//move unused components out

//move components/ to extension/components/
...
promise,
//chai, contract check
////(set Main.isTest = false and lock it!)
...
//move shadow out:
move glsl out
move shadow to extension


refactor:
////move animation to src/component/
//rename: lite_xxx, all_xxx
//rename karma files




//set Main.isTest = false and lock it if no chai!
...
////move renderTarget out(can remain)
...
////support compress function:
class add "className" field
...
**** DONE support package config options
CLOSED: [2016-12-12 一 16:18]


add:
//lite.debug.js+innerLib.js
...
refactor:
//refactor packageLite by packageCustom
//move all inner requires out
//rename all to core
//clean gulp task code


//fix sample->js path


//pass all unit test, render test
...
* TODO 疑问
** TODO how to solve render test problem(pixel?)
ask @mochuan:
Hilo don't solve it too
