* 今日计划
** TODO prepare software


** TODO prepare develop environment

** TODO parse fbx(v0.8.0)
...
** DONE demo
CLOSED: [2016-10-28 五 08:08]
//learn python


//prepare python environment

//can parse fdx by python

//pass python unit test


improve scene graph:
//refactor .wd, refer to gltf


parse fbx demo:
//import scene, export scene

//test parse mesh data

//convert .wd(text file) to .wd + .bin:

** DONE see the gongzonghao
CLOSED: [2016-10-27 四 20:00]



** DONE 3d the race game
CLOSED: [2016-11-11 五 16:20]
//optimize event->remove

//support key frame->texture
...
*** DONE build server
CLOSED: [2016-11-08 二 08:01]


*** DONE finish front end
CLOSED: [2016-11-11 五 15:01]
//integrate with 3d

//show canvas role name

//show 50+ roles

//uiObject handle isVisible
...
mock socket(1000)
why disconnect?(max connection:200!)
...
////add chinese bitmap font

////*** TODO fix frp
fix timeout(timeout .concat() not work!)

add delay



** DONE parse fbx(v0.8.0)
CLOSED: [2016-12-01 四 19:38]
continue parse:
*** DONE parse material
CLOSED: [2016-11-14 一 20:53]
...
*** DONE parse transform
CLOSED: [2016-11-14 一 20:13]



*** DONE use blender
CLOSED: [2016-11-16 三 19:57]
//create .fbx to test
...
*** DONE optimize:
CLOSED: [2016-11-16 三 20:38]
if texture asset same, share its texture!





*** DONE parse animation:
CLOSED: [2016-11-24 四 19:45]
key frame
support transform
...
//get correct by test webgl.fpx

//how to correct multi animations!(actions)
...
test two models with key frame animations
learn blender->action
...
morph:only support wd:
//fix md2 converter

//convert multi indices->consider morphTargets

//compress morphTargets
...
refactor:
//refactor tool->converter
//refactor WDParserSpec
////refactor WDAssemblerSpec
//extract WDMorphDataParserUtils
//rename src->morphTargets to morphFrames(note IWDInterface)
...
*** DONE parse camera
CLOSED: [2016-11-27 日 15:08]
//refactor WDParserSpec


fix camera + animation:
add test scene by blender and pass(may add custom matrix class)
(not consider pivot):
camera + static object
key frame object + key frame camera


try pass duck,webgl,Ske fbx
...
refactor:
//rename fbx->Xhelper to Xutils
//refactor WDParser
//IWDInterface->add ForAssembler

*** DONE parse light
CLOSED: [2016-11-27 日 18:36]
////support shadow:
fbxMesh:cast/receive shadow
light:shadow related


//refactor:
split IWDInterface to IWDAssemblerData, IWDParserData
refactor WDAssembler->isXXX



*** DONE parse meta data
CLOSED: [2016-11-28 一 04:15]


////*** TODO run test
test more fbx models


*** DONE fix md2 converter
CLOSED: [2016-11-24 四 19:46]




*** DONE fix obj converter
CLOSED: [2016-11-28 一 19:48]



//optimize:
only remain its own primitive data
add gulp task: only get used primitive data by indices
////or: to obj, children share the vbo of the parent


refactor:
extract SingleModelConverter
clear code(IWDXXX, ...)
*** DONE add gltf converter
CLOSED: [2016-11-29 二 15:19]
...
*** DONE pass all tests
CLOSED: [2016-11-30 三 11:34]
//pass unit tests

//pass render tests

//test all converters:
test filter with fbx


*** DONE refactor converter gulp task
CLOSED: [2016-12-01 四 10:24]
//integrate with fbx python converter

//copy resource to dest dir
...
** DONE refactor
CLOSED: [2016-12-01 四 11:00]
//refactor code
//refactor fbx converter:
remove threejs related code




** DONE compare wonderjs and threejs
CLOSED: [2016-11-18 五 12:00]
//write what wonder can do and can't do
...
** DONE update typescript to 2.1
CLOSED: [2016-12-01 四 11:07]
...
** TODO publish hilo3d v0.1.0(1.1 finish)(1.18 must publish!)
*** TODO move min files from wonder.js to wonder


*** TODO prepare hilo3d
create hilo3d project in gitlab

discuss with wang,jiang:
cut works(mainly with wang)
...
*** target
- support blender/maya?->engine->run flow
- can run in mobile
- has min feature
give basic package:
geometry+material+camera(need confirm with jiang)
- publish to tian mao front developers



*** TODO support run in mobile(ios 8+, android 5.0+)(v0.9.0)
**** TODO test uc4->webgl support, give a report
**** TODO discuss with jiang about engine development
  specify engine feature, priority, goal

  give a dev->publish timeline

*** TODO test
test in ios WKWebView(@fu ding)
...
change character to be LightMaterial


** TODO finish fbx scene page on zebraz

*** TODO use blender to create fbx scene(based on exist fbx?)
create terrain:
https://www.blender.org/manual/editors/3dview/object/transform/transform_control/proportional_edit.html#fig-view3d-transform-landscape


*** TODO show 3d scene by wonder.js on zebra
use mui4.0


** TODO optimize(v0.10.0)
*** TODO cpu optimize
optimize transform:
optimize Transform->state?
check is the same in setting position,scale,rotation, localXXX
...
* TODO 疑问
** blender
- how to repeat texture and export to fbx??? i can't export to fbx!!!
- why export keyframe->rotation to fbx is wrong(error curve data:the rotation curve data components' count are different)???
This is a known issue. When animating rotation about any axis other than the global x, y, z-axes,
Blender tends to calculate some odd interpolation between the keyframes (actually, it is the direct
result of the interpolation that you see in the f-curve editor). However, there is a fix/workaround
that I use, which I would describe as "making the local axis global".  

http://www.fsdeveloper.com/forum/threads/blender-local-rotation-issue.428512/
...
key frame:
support more property and more interpolation,mode...(e.g. cubic interpolation)
