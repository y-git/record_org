* 今日计划
** TODO finish nian huo jie project

* 完成事项
//** TODO 1.18 publish Hilo3D
//*** TODO share01
confirm date (wed or thus)

collect issues


write document to ata

//*** TODO write use document for developers(tian mao)


** TODO nian huo jie project
communicate with ued:(@入也, 180-7276-7533 郑仪逊)
record needed features, optimizations(+ or -)

no light
skeleton animation
.fbx scene(some static objects, 1 or 2 animation objects)
play sound


*** DONE give timeline to @zu
CLOSED: [2016-12-13 二 08:21]


*** DONE refactor
CLOSED: [2016-12-13 二 10:27]
move component back to src/component/

refactor DeviceManager:use root


*** DONE demo study
CLOSED: [2016-12-27 二 13:51]
**** DONE run in mobile
CLOSED: [2016-12-20 二 09:00]
//read references

//build mobile develop environment


//show on mobile


support touch event(use hammer?):
////use hammer to handle touch and mouse event

//pass mouse,dom

//pass event script

//pass custom event test:
////test parent-child 



//add touch test:
//not trigger POINTSCALE event
//add touch event test
//add touch event script test

//add tap

//pass arcball, fly unit test

//pass button


//refactor:
////touch drag = touch move
//remove listener
//clean
//rename p_type
...
////add touch camera(arcball? no scale)
...
////refer to timespace(cp timespace to reference):
pixel ratio?
support orientation
...
//test audio, model, arcball camera:

***** DONE show debug panel for mobile test
CLOSED: [2016-12-20 二 09:00]

//show debug panel(Main)
...
**** DONE support basic skeleton animation
CLOSED: [2016-12-27 二 13:51]
//modify runtime:
write ModelGeometry
write ShaderLib
...
//.gltf convert to .hilo3d

//fix gltf:(.gltf only can contain one skin animation?)
//move all joint animation data to one "animation" field?
...
//fix: if not play animation, show obj

//fix mesh error

fix monster->end animation
...
////fix end animation bug



fix:
////"JOINT"->componentType should be 5123 instead of 5126 ???
fix unit test!
...
refactor:
//refactor GLTFToHilo3D
//clean code
//add unit test
//refactor ModelGeometry(move merge out?)
...
*** TODO support basic skeleton animation
refactor:
//decompose Hilo3DAssemblerSpec

////refactor SkinSkeletonAnimation: use Collection instead of Array
...
**** DONE modify .fbx
CLOSED: [2016-12-28 三 20:26]
//fbx support skinSkeleton
...
//test in mobile(one animation)


//clone to three models to test
...
//how to load .hilo3d, .bin, .png(put img together):
////add nodejs version(all, lite, custom)




//use ar(other phone) to test
...
fix:
//after biannian restore

//when move out, invisible
...
//use .js instead of .hilo3d





////adjust yuanbao model!


//update all.js

//use js to import model
need test online
...
//when move over, play again

//solve slow problem


//test code in ios,android!




//pass android ar matrix!
...
refer to 2d:
////yuanbao not move?
bainian is in center?


////ask @xiaozu, jump to bainian?
...
fix tips in arPage.js @xiaozu



update model normal data
fix
...
fix tips (loading, duizhun) info:
now show duizhun when success ar
...
//loading optimize(show)



//forbidden touch camera

//model should rotate around ar matrix



fix:
//when frequency move out/in, cat may not disappear!
...
//test in mobile(two animation)


//communicate:
remove light,camera data
...
test on android(in tianmao, taobao)


test on ios(in tianmao, taobao)



////buy iphone
buy iphone5, meizhu, google?



**** TODO package .mobile.js for mobile



////*** TODO fix converter, scene graph, support basic material
fix fbx converter:
support BasicMaterial(by set command option)


fix .hilo3d scene graph:
support BasicMaterial


refactor converter:
clean code(gulp_task)
...
*** TODO finish project
//switch to wonder(3 commit):
update frp, commonLib

move skin sample to wonder and pass it
...
fix tool->converter:
remove source/*.xxx when convert in isEmbedded(refer to hongbao)

optimize tool->converter->isEmbedded:
if image/buffer not exist, warn
...
fix skin animation:
////removeChild still has bug!
play the hongbao->animation, at the begin of the second time it will shake!



fix hongbao:
box collider is wrong in ios,android(the triggered shape's position is wrong!):
refer to lafite-54444 project -> mine -> 3d->index->collider bug!.js:
set BasicCameraController->worldToCameraMatrix in each loop cause the problem:
    _updateCameraMatrixByAr: function(){
        // if(this._arMatrixUpdateCount < this._arMatrixUpdateRate){
        //     this._arMatrixUpdateCount++;
        //
        //     return;
        // }
        //
        // this._arMatrixUpdateCount = 0;

        var data = this._arData[0],
            pMatrix = Hilo3D.Matrix4.create(new Float32Array(
                data.projection
            )),
            mMatrix = Hilo3D.Matrix4.create(new Float32Array(
                data.modelmatrix
            ));

        var vMatrix = mMatrix;

        var controller = this._camera.getComponent(Hilo3D.BasicCameraController);

        controller.pMatrix =pMatrix;

        //this cause bug!
        controller.worldToCameraMatrix =vMatrix;
    },
...
fix:
set collider->center in loop not update?




restore Hilo3DLoader
restore ShadowManager



refactor:
remove Skin animation-> isTimeExceed5000 logic

refactor CommonShaderLib->data.vMatrix/pMatrix(add support set entityObject->vMatrix/pMatrix instead of camera.vMatrix/pMatrix)
(consider has children)


test or refactor event->ANIMATION_STOP


test Camera->isUserSpecifyThePMatrix

test Billboard->directionEulerAngles

test ThreeDTransform->setLocalToWorldMatrix
...
skin animation first play is slow and isn't complelte!
(refactor LayerKeyFrameController,JointKeyFrameController->_isFirst judge logic!)
...
study jiang's project(use inka3D)
http://www.inka3d.com/

how to reduce engine size
learn useful things
...
refactor: move Material->shading to StandardLightMaterial
...
fix tool->converter:
if in source/xxx/ folder, not copy to dest/xxx.hilo3d !



restore Transform->setXXXTransformState

restore CompileConfig->isCompileTest to true(if false, how to pass unit test?)
...
glsl support @require mechanism
...
solve how to extend by user:(refer to unity)
...
refactor:
split tool->converter->convert indices->Converter
split tool->converter->CompressorSpec
refactor BoneMatrix with Transform(add corresponding unit test)


test:
test Hilo3DLoader(e.g., convert image to base64)



skeleton:
fix:
handle JointKeyFrameController->saveZeroTimeFrameData->handle first time !== 0!
...
fix tool->converter:
if embedded, gltf should still be embedded!
...
*** TODO publish v0.10.0
** TODO optimize(v0.11.0)
...
** TODO add particle system(v0.12.0)
** TODO add hdr post effect(v0.13.0)(move to extension)
** TODO add AssetDatabase to support aync load asset(v0.14.0)(move out to be a project in wonder group)
** TODO finish projects to apply engine
*** TODO in pc
**** TODO write a big demo(pc)
...
*** TODO in mobile
**** TODO tao fu wa
**** TODO bainian(add more effect, show three cats)
