...
*** TODO support share the same texture
different model with the texture of the same id should share the same texture!?
...
** DONE update editor version
CLOSED: [2018-05-22 Tue 16:54]



** DONE add load asset(v)
CLOSED: [2018-06-12 Tue 20:12]
*** DONE learn
CLOSED: [2018-05-23 Wed 11:49]
http://what-when-how.com/Tutorial/topic-4863q9vm8/HTML5-Game-Development-Insights-20.html
...
**** DONE define wd format
CLOSED: [2018-05-25 Fri 14:50]





////**** TODO convert obj to .wd
**** TODO convert gltf to .wd


check no multi primitives
...
not support TEXCOORD_1

////not support multi primitives per mesh
support multi primitives per mesh
...
Images:
only support "a URI to an external file in one of the supported images formats"
    uri must exist
...
////support extensions->material_common(with only diffuse,specular, shininess data)

use base color+	metallic value+...:
use base color as diffuse color
use metallic value as specular color
use roughness value as shininess
...
***** TODO converter with .obj,.mtl, image files data
result is string + .bin string?


////***** TODO node tool: converter with load obj model
result is .wd + .bin file


***** TODO optimize: result should be organized as batch data
...
Default Material
The default material, used when a mesh does not specify a material, is defined to be a material with no properties
specified. All the default values of material apply. Note that this material does not emit light and will be black
unless some lighting is present in the scene. 
...
//batch set transform parent:
set in last!


//batch set transform data:
not consider parent and update(set parent last, now no parent!)




material:
only load diffuse. specular and shininess use default one!
...
***** TODO directly assemble wd from coverted result!
...
*** DONE pass test
CLOSED: [2018-06-01 Fri 09:33]
**** DONE test convert gltf
CLOSED: [2018-05-31 Thu 10:50]


fix multi primitives:
//pass compile

convert geometrys: not handle multi primitives
convert geometry gameObjectIndices: not handle multi primitives
...
**** DONE test compute normals
CLOSED: [2018-06-01 Fri 09:33]


*** DONE upgrade reason, bs-platform
CLOSED: [2018-06-01 Fri 17:35]

**** DONE remove js boolean, use reason boolean instead; remove Js.to_bool, Js.Boolean.to_js_boolean
CLOSED: [2018-06-01 Fri 17:35]



*** DONE optimize: benchmark batch create vs create one by one
CLOSED: [2018-06-03 Sun 12:17]

result:
"batch create" is faster the "create one by one"


*** DONE optimize
CLOSED: [2018-06-04 Mon 15:42]

now "compute vertex normals" when get normals(when render)


*** DONE refactor
CLOSED: [2018-06-05 Tue 09:12]
//move DirectorSystem to api/DirectorAPI
...
*** DONE add more assemble logic
CLOSED: [2018-06-05 Tue 22:14]
...
***** DONE transform support rotate, scale
CLOSED: [2018-06-06 Wed 18:29]




////***** TODO transform support lookAt
...
***** DONE fix getRotation bug
CLOSED: [2018-06-06 Wed 20:02]
...
***** DONE asset->transform
CLOSED: [2018-06-06 Wed 18:29]


**** DONE fix texture->flipY
CLOSED: [2018-06-07 Thu 17:35]
basicSource
...
**** DONE fix canvas bug
CLOSED: [2018-06-07 Thu 20:44]



////***** TODO fix transform bug
render should set rotation,scale data
...
add translate, translateLocal, rotate, rotateLocal, lookAt, ...


**** DONE cameras
CLOSED: [2018-06-08 Fri 10:19]
...
////***** TODO optimize multi primitives
if use the same mesh(multi primitives), share the same node!
...
*** DONE fix
CLOSED: [2018-06-11 Mon 20:26]

//To achieve correct rendering, WebGL runtimes must disable such conversions by setting UNPACK_COLORSPACE_CONVERSION_WEBGL
flag to NONE:
...
////texture:
filter not allow use mipmap(contract check)
(
Mipmapping Implementation Note: When a sampler's minification filter (minFilter) uses mipmapping
(NEAREST_MIPMAP_NEAREST, NEAREST_MIPMAP_LINEAR, LINEAR_MIPMAP_NEAREST, or LINEAR_MIPMAP_LINEAR), any texture referencing
the sampler needs to have mipmaps, e.g., by calling GL's generateMipmap() function. 
)




////*** TODO customGeometry add smooth
////compute flat/vertex normals?
////if none, not compute normals
...
*** DONE add name
CLOSED: [2018-06-12 Tue 19:44]
node, mesh, material(light material), texture and image(consider texture asset) add name
...
** DONE optimize: show model
CLOSED: [2018-06-12 Tue 21:35]
...
** DONE generate .wd from scene(engine do this)
CLOSED: [2018-06-19 Tue 09:47]
...
*** DONE generate embeded .wd
CLOSED: [2018-06-19 Tue 09:47]

**** DONE geometry:
CLOSED: [2018-06-17 Sun 21:25]
//finish draft
...
//commit:
rename GameObjectAPI->unsafeGetGameObjectCustomGeometryComponent to unsafeGetGameObjectGeometryComponent
...
////boxGeometry + customGeometry




contract check:
nodes,meshes, ... shouldn't has slot!
...
//shared geometry:
gltf add node->extension->material
fix generate
...
test buffer data(vertices, ... data)


primitives->attributes can has no TEXCOORD_0
...
**** DONE benchmark assemble
CLOSED: [2018-06-17 Sun 21:29]



**** DONE material
CLOSED: [2018-06-18 Mon 20:50]
...
**** DONE refactor material
CLOSED: [2018-06-19 Tue 05:47]
...
asset->test to be integration test
...
** DONE optimize generate wd
CLOSED: [2018-06-19 Tue 12:14]
...
** DONE fix: set new created texture
CLOSED: [2018-06-21 Thu 06:31]
...
*** TODO editor
publish wonder.js npm!


need judge geometry has texCoord?
show info modal



** DONE api add functions
CLOSED: [2018-06-21 Thu 06:33]
////BasicSourceTextureAPI:
add initTexture


BasicMaterialAPI:
rename hasLightMaterialMap to hasBasicMaterialMap



** DONE add light
CLOSED: [2018-06-22 Fri 09:35]
...
//change ambient to be scene data:


//test:
    add scene-> ambient light test
...
**** TODO convert gltf glb to wd



**** TODO generate wd glb from scene
...
** DONE finish all TODO
CLOSED: [2018-06-22 Fri 20:15]



** DONE pass coverage
CLOSED: [2018-06-22 Fri 20:32]



** DONE pass code climate
CLOSED: [2018-06-22 Fri 22:37]



** DONE pass ci
CLOSED: [2018-06-22 Fri 22:51]



** DONE publish
CLOSED: [2018-06-22 Fri 23:11]
...
** TODO editor fix name
(use engine name api instead of editor state->name)
...
** TODO VC gltf model wrong
the road,railway show wrong!!! why????


(other gltf models(include cad models) show correctly!)





** TODO generate wd-> 2CylinderEngine gltf model wrong
not the same as origin gltf model!!!
